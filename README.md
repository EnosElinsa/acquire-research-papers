...
Discovery paginates venue/year sources and writes `coverage.jsonl`, `candidates.jsonl`, `evidence-packets.jsonl`, `pending-metadata.csv`, and `discovery-manifest.json`; it performs no publisher download and does not freeze a selection. Codex reviews title and abstract evidence, while keywords are optional and full text is not required. `review corpus` validates `review-decisions.jsonl` against the request and immutable discovery artifact hashes, applies quotas, and writes `selected-papers.jsonl` plus `selection-manifest.json`. Once the preferred total and all quotas are satisfied, a hash-audited early stop may leave overflow packets pending; they cannot enter the selection. Acquisition binds its output directory before publisher contact, consumes only that frozen selection, and never adds or removes papers. It writes verified pairs, `selection-binding.json`, `acquisition-manifest.jsonl`, `manual-download.csv`, `retryable-downloads.csv`, and `delivery-manifest.json`.
